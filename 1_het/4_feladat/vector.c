...
void vector_add(float *A, float *B, float *result, size_t size) {
  ...
  // WRITE BUFFERS
  clEnqueueWriteBuffer(command_queue, a_buffer, CL_FALSE, 0,
                       size * sizeof(float), A, 0, NULL, NULL);
  clEnqueueWriteBuffer(command_queue, b_buffer, CL_FALSE, 0,
                       size * sizeof(float), B, 0, NULL, NULL);
  ...
  // Host buffer <- Device buffer
  // READ BUFFER
  clEnqueueReadBuffer(command_queue, result_buffer, CL_TRUE, 0,
